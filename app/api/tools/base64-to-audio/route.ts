...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    if (!base64) return NextResponse.json({ error: 'Provide base64 audio string' }, { status: 400 });
    ...
    return NextResponse.json({ error: error.message || 'Failed to decode audio' }, { status: 500 });
  }
}
