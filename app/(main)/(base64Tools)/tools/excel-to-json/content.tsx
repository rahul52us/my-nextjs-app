...
export default function ExcelToJsonContent() {
  ...
  const decodeBase64ToBlob = (
    ...
    try {
      ...
      if (byteArray.length === 0) {
        setError(
          "Decoded byte array is empty. Please check the Base64 string.",
        );
        return null;
      }
  ...
  const handleDownloadJson = (sheetName: string) => {
    ...
    saveAs(
      blob,
      `${sheetName}_rows_${range.start + 1}-${range.end + 1}_selected_columns.json`,
    );
    setError(null);
    toast({
  ...
  const handleDownloadAllJsons = async () => {
    ...
    if (!hasData) {
      setError(
        "No sheets with selected columns and valid data in the specified row range.",
      );
      toast({
        title: "No data to download",
        description:
          "Please select columns and valid row ranges for at least one sheet.",
        status: "error",
        duration: 3000,
        isClosable: true,
        position: "top-right",
    ...
    try {
      ...
      saveAs(
        content,
        `all_sheets_rows_${startRow}-${endRow || "end"}_${new Date().toISOString().replace(/[^\d]/g, "").slice(0, 14)}.zip`,
      );
      setError(null);
      toast({
      ...
      <Card p={6} borderRadius="lg" boxShadow="lg" bg={cardBg}>
        ...
        <VStack spacing={6} align="stretch">
          ...
          <Text fontSize="sm" color="gray.500" textAlign="center" mt={2}>
            Upload an Excel file (.xlsx, .xls) or a .txt file with a Base64
            string, or paste Base64 directly to process and preview.
          </Text>
        </VStack>
      </Card>
            ...
            {isLoading ? (
              <VStack spacing={4} py={10} align="center">
                ...
                <Text color={textColor}>
                  Loading file... {Math.round(loadingProgress)}%
                </Text>
              </VStack>
            ) : previewContent ? (
                ...
                <VStack spacing={6} align="stretch">
                  ...
                  <Card p={4} borderRadius="lg" boxShadow="md" bg={cardBg}>
                    <HStack spacing={4} flexWrap="wrap" justify="space-between">
                      <HStack spacing={2}>
                        <Tooltip
                          label={`Download JSON for ${selectedSheet}`}
                          hasArrow
                        >
                          <Button
                            colorScheme="teal"
                        ...
                        <Tooltip label="Download all sheets as zip" hasArrow>
                          <Button
                            ...
                            isDisabled={
                              !previewContent ||
                              !Object.values(selectedColumns).some(
                                (cols) => cols.length > 0,
                              )
                            }
                            size="sm"
