...
const BinaryToHexContent: React.FC = () => {
  ...
  const convertBinaryToHex = (binary: string): string => {
    try {
      const cleanBinary = binary.replace(/\s/g, "");
      if (!/^[01]*$/.test(cleanBinary)) {
        return "Invalid binary input. Only 0s and 1s allowed.";
      }
      const hex = parseInt(cleanBinary, 2).toString(16).toUpperCase();
      return hex;
    } catch {
  ...
  const handleConversion = () => {
    setLoading(true);
    setHexOutput("");
    const output = convertBinaryToHex(binaryInput);
    setHexOutput(output);
    setLoading(false);
  };
    ...
    <Box p={4} bg="transparent" color={textColor}>
      ...
      <VStack spacing={6} align="stretch">
        <FormControl>
          ...
          <Textarea
            placeholder="Paste your binary string here (e.g., 11010110 10101010)"
            value={binaryInput}
        ...
        <Box>
          ...
          >
            {loading ? (
              ...
              hexOutput || "Your converted binary will appear here."
            )}
          </Box>
          {hexOutput && (
...
export default BinaryToHexContent;