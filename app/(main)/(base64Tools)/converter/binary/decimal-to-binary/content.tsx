...
const DecimalToBinaryContent: React.FC = () => {
  ...
  const convertDecimalToBinary = (decimal: string): string => {
    try {
      const cleanDecimal = decimal.trim();
      if (!/^-?\d+$/.test(cleanDecimal)) {
        return "Invalid decimal input. Please enter a valid integer.";
      }
      const num = BigInt(cleanDecimal);
  ...
  const handleConversion = () => {
    setLoading(true);
    setBinaryOutput("");
    const output = convertDecimalToBinary(decimalInput);
  ...
  const readTextFile = (file: File) => {
    ...
    reader.onload = () => {
      ...
      if (content) {
        setDecimalInput(content);
        toast({
          title: "File uploaded successfully!",
          description: "Decimal content loaded from the file.",
          status: "success",
          duration: 3000,
          isClosable: true,
        });
    ...
    <Box p={4} bg={bgColor} color={textColor}>
      ...
      <VStack spacing={6} align="stretch">
        <FormControl>
          ...
          <Textarea
            placeholder="Paste your decimal number here (e.g., 42)"
            value={decimalInput}
        ...
        <HStack spacing={4} justify="space-between">
          ...
          <Button
            colorScheme="red"
            size="lg"
            display={decimalInput?.trim() ? undefined : "none"}
        ...
        <Box>
          ...
          >
            {loading ? (
              ...
              binaryOutput || "Your converted number will appear here."
            )}
          </Box>
          {binaryOutput && (
...
export default DecimalToBinaryContent;