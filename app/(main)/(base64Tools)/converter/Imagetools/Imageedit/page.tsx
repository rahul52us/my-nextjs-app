...
const AIasist: React.FC = () => {
  ...
  const renderCanvas = useCallback((useOriginal = false) => {
    ...
    if (!useOriginal) {
      ...
      if (foregroundImage && filters.bgBlur > 0) {
        ctx.save();
      ...
      <div className="flex h-[calc(100vh-64px)]">
        <aside className="w-85 bg-white border-r border-slate-200 overflow-y-auto p-6 space-y-8 no-scrollbar">
          <section className="space-y-3">
             ...
             <button 
                onClick={() => processAI('remove')} 
                disabled={isProcessing || !image} 
                className="w-full flex items-center justify-between p-4 bg-slate-900 rounded-2xl text-white hover:bg-indigo-600 transition-all shadow-lg shadow-slate-200 group disabled:opacity-50"
             >
             ...
             <button 
                onClick={() => processAI('portrait')}
                disabled={isProcessing || !image}
                className="w-full flex items-center justify-between p-4 bg-white border border-slate-200 rounded-2xl text-slate-900 hover:border-indigo-600 transition-all group disabled:opacity-50"
             >
            ...
            <div className="grid grid-cols-3 gap-2">
              {['Cyberpunk', 'Sahara', 'Forest', 'Noir', 'Duotone', 'Reset'].map(preset => (
                <button 
                  ...
                  className="py-2 rounded-xl text-[10px] font-black uppercase tracking-tighter border border-slate-100 hover:border-indigo-500 hover:text-indigo-600 transition-all bg-slate-50/50"
                >
                  {preset}
                </button>
              ))}
          ...
          <section className="space-y-6">
            ...
            <Slider label="Soft Focus" value={filters.blur} max={15} onChange={(v) => updateFilters({...filters, blur: v})} />
          </section>
        ...
        <main className="flex-1 bg-[#F8FAFC] p-12 flex items-center justify-center relative">
          {!image ? (
            <div {...getRootProps()} className="w-full max-w-2xl aspect-video bg-white border-2 border-dashed border-slate-200 rounded-[48px] flex flex-col items-center justify-center gap-6 cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group shadow-2xl">
              ...
              <div className="text-center">
                ...
                <p className="text-slate-400 font-bold mt-1">Drag and drop any image to launch VisionStudio</p>
              </div>
            </div>
          ) : (
            <div className="relative group max-h-full">
              ...
              {isProcessing && (
                <div className="absolute inset-0 bg-white/90 backdrop-blur-xl rounded-3xl flex flex-col items-center justify-center z-50">
                  ...
                  <p className="text-lg font-black text-slate-800 uppercase tracking-widest animate-pulse text-center">AI Intelligence Loading...<br/><span className="text-[10px] opacity-50">Downloading Models (18MB)</span></p>
                </div>
              )}
            </div>
          )}
      ...
      {showQRModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[100] flex items-center justify-center p-6" onClick={() => setShowQRModal(false)}>
           <div className="bg-white p-10 rounded-[40px] max-w-sm w-full shadow-2xl animate-in zoom-in-90" onClick={e => e.stopPropagation()}>
             ...
             <p className="text-slate-400 text-sm mb-8 font-medium italic">Embed a digital link into your work.</p>
             <div className="bg-slate-50 p-8 rounded-3xl flex justify-center mb-8 border border-slate-100 shadow-inner">
                <QRCodeSVG value={qrValue} size={160} />
             </div>
             <input value={qrValue} onChange={e => setQrValue(e.target.value)} className="w-full p-4 bg-slate-100 rounded-2xl text-sm font-bold border-none focus:ring-2 focus:ring-indigo-600 mb-8" />
...
const ToolIconButton = ({ icon, onClick }: { icon: React.ReactNode, onClick: () => void }) => (
  <button 
    onClick={onClick} 
    className="aspect-square bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-center hover:bg-white hover:border-indigo-400 hover:text-indigo-600 hover:shadow-xl transition-all active:scale-90"
  >
    {icon}
  </button>
);
