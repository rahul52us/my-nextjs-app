...
const Base64PdfContent = () => {
    ...
    const extractMimeAndData = (input: string) => {
        ...
        if (matches) {
            return {
                mimeType: matches[1],  // Extracts MIME type (e.g., image/png)
                base64Data: matches[2], // Extracts the Base64 encoded data
            };
        }
    ...
    // Handle the convert action, triggering file download
    const handleConvert = () => {
        if (!base64.trim()) {
    ...
    // Preview the Base64 content if it's an image
    const previewBase64 = (input: string) => {
        const { mimeType, base64Data } = extractMimeAndData(input);
        setFileType(mimeType);

        if (mimeType.startsWith("image")) {
            setWarningMessage(null); // Clear any previous warnings for valid images
...
export default Base64PdfContent;
